{*
    ...
    Данные: fn_rlsv_autopost_get_feature_analytics() → $rlsv_pinterest (agg/spark/donut в $rlsv_pinterest.dash).
*}
{capture name="mainbox"}
{assign var="d"  value=$rlsv_pinterest.dash}
{assign var="lb" value=$rlsv_pinterest.labels}
{assign var="fm" value=$rlsv_pinterest.feature_map}
<div id="rlsv-pinterest" class="rlsv-ap rlsv-bento-wrap">
    <style>
        #rlsv-pinterest.rlsv-bento-wrap { color:#465363; }
        #rlsv-pinterest .rlsv-bento { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:22px; }
        #rlsv-pinterest .rlsv-wide { grid-column:1 / -1; }
        #rlsv-pinterest .rlsv-sec { margin:4px 2px 10px; font-size:11px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:#46aaf2; display:flex; align-items:center; gap:7px; }
        #rlsv-pinterest .rlsv-sec.rlsv-reels { color:#465363; }
        #rlsv-pinterest .rlsv-sec.rlsv-story { color:#f2464e; }
        #rlsv-pinterest .rlsv-b { background:#fff; border:1px solid #e1e9ee; border-radius:14px; padding:16px; box-shadow:0 1px 3px 0 rgba(70,83,99,.08); box-sizing:border-box; }
        #rlsv-pinterest .rlsv-hero { background:linear-gradient(135deg,#eef5fd 0%,#ffffff 60%); border-color:#cfe4fb; position:relative; overflow:hidden; }
        #rlsv-pinterest .rlsv-cap { font-size:13px; font-weight:500; color:#5b6b7d; display:flex; align-items:center; gap:8px; }
        #rlsv-pinterest .rlsv-num { font-size:30px; font-weight:700; color:#2b3648; line-height:1.1; letter-spacing:-.5px; margin-top:6px; }
        #rlsv-pinterest .rlsv-num.rlsv-sm { font-size:22px; }
        #rlsv-pinterest .rlsv-num.rlsv-xs { font-size:18px; }
        #rlsv-pinterest .rlsv-lbl { font-size:10px; letter-spacing:.06em; text-transform:uppercase; color:#8a97a3; }
        #rlsv-pinterest .rlsv-ico { width:30px; height:30px; border-radius:50%; background:#e8f3fd; display:inline-flex; align-items:center; justify-content:center; flex:none; }
        #rlsv-pinterest .rlsv-ico svg { display:block; }
        #rlsv-pinterest .rlsv-head { display:flex; justify-content:space-between; align-items:flex-start; }
        #rlsv-pinterest .rlsv-trend { display:inline-flex; align-items:center; gap:4px; font-size:12px; font-weight:700; padding:3px 8px; border-radius:9px; }
        #rlsv-pinterest .rlsv-trend.rlsv-up { color:#27ae60; background:rgba(39,174,96,.12); }
        #rlsv-pinterest .rlsv-trend.rlsv-down { color:#f2464e; background:rgba(242,70,78,.12); }
        #rlsv-pinterest .rlsv-split { display:flex; }
        #rlsv-pinterest .rlsv-split > div { flex:1; display:flex; align-items:center; justify-content:space-between; }
        #rlsv-pinterest .rlsv-split > div:last-child { padding-left:14px; border-left:1px solid #edf2f5; }
        #rlsv-pinterest .rlsv-split > div:first-child { padding-right:14px; }
        #rlsv-pinterest .rlsv-two { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
        #rlsv-pinterest .rlsv-two > div + div { border-left:1px solid #edf2f5; padding-left:14px; }
        #rlsv-pinterest .rlsv-break { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; padding-top:12px; margin-top:12px; border-top:1px solid #edf2f5; }
        #rlsv-pinterest .rlsv-bkit { display:flex; flex-direction:column; align-items:center; gap:5px; }
        #rlsv-pinterest .rlsv-bkdot { width:9px; height:9px; border-radius:50%; }
        #rlsv-pinterest .rlsv-bkv { font-size:13px; font-weight:700; color:#2b3648; }
        #rlsv-pinterest .rlsv-rows > div { display:flex; justify-content:space-between; align-items:center; }
        #rlsv-pinterest .rlsv-rows > div + div { margin-top:10px; }
        #rlsv-pinterest .rlsv-fid { display:inline-block; margin-left:6px; padding:0 6px; border-radius:8px; background:#edf2f5; color:#46aaf2; font-size:10px; font-weight:700; vertical-align:middle; }
        #rlsv-pinterest .rlsv-pinterest-legend { border:1px dashed #d3dde3; border-radius:10px; padding:9px 12px; margin-bottom:16px; font-size:12px; color:#465363; background:#fafdff; }
        #rlsv-pinterest .rlsv-recent { border:1px solid #e1e9ee; border-radius:12px; padding:12px 14px; margin-bottom:12px; background:#fff; }
        #rlsv-pinterest .rlsv-rtype { display:inline-block; padding:2px 10px; border-radius:10px; font-size:12px; background:#e8f3fd; color:#46aaf2; }
        #rlsv-pinterest .rlsv-rtype.rlsv-reels { background:#fdeef0; color:#f2464e; }
        #rlsv-pinterest .rlsv-rtype.rlsv-story { background:#fef0dd; color:#b5670a; }
        #rlsv-pinterest .rlsv-rmini { display:flex; flex-wrap:wrap; gap:6px 16px; margin-top:8px; font-size:12px; color:#5b6b7d; }
        #rlsv-pinterest .rlsv-muted { color:#8a97a3; font-size:12px; }
        @media (max-width:480px){ #rlsv-pinterest .rlsv-two { grid-template-columns:1fr; } #rlsv-pinterest .rlsv-two > div + div { border-left:none; padding-left:0; border-top:1px solid #edf2f5; padding-top:10px; } }
    </style>
    ...
    {else}
        ...
        {if $rlsv_pinterest.media}
            <div class="rlsv-sec">{__("rlsv_autopost.tab_by_product")}</div>
            {foreach from=$rlsv_pinterest.media item="m"}
...
{capture name="mainbox_title"}{__("rlsv_autopost.pinterest_analytics")}{/capture}
