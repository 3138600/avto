{*
    ...
    Данные: fn_rlsv_autopost_get_feature_analytics() → $rlsv_tg (agg/spark/donut в $rlsv_tg.dash).
*}
{capture name="mainbox"}
{assign var="d"  value=$rlsv_tg.dash}
{assign var="lb" value=$rlsv_tg.labels}
{assign var="fm" value=$rlsv_tg.feature_map}
<div id="rlsv-telegram" class="rlsv-ap rlsv-bento-wrap">
    <style>
        #rlsv-telegram.rlsv-bento-wrap { color:#465363; }
        #rlsv-telegram .rlsv-bento { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:22px; }
        #rlsv-telegram .rlsv-wide { grid-column:1 / -1; }
        #rlsv-telegram .rlsv-sec { margin:4px 2px 10px; font-size:11px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:#46aaf2; display:flex; align-items:center; gap:7px; }
        #rlsv-telegram .rlsv-sec.rlsv-reels { color:#465363; }
        #rlsv-telegram .rlsv-sec.rlsv-story { color:#f2464e; }
        #rlsv-telegram .rlsv-b { background:#fff; border:1px solid #e1e9ee; border-radius:14px; padding:16px; box-shadow:0 1px 3px 0 rgba(70,83,99,.08); box-sizing:border-box; }
        #rlsv-telegram .rlsv-hero { background:linear-gradient(135deg,#eef5fd 0%,#ffffff 60%); border-color:#cfe4fb; position:relative; overflow:hidden; }
        #rlsv-telegram .rlsv-cap { font-size:13px; font-weight:500; color:#5b6b7d; display:flex; align-items:center; gap:8px; }
        #rlsv-telegram .rlsv-num { font-size:30px; font-weight:700; color:#2b3648; line-height:1.1; letter-spacing:-.5px; margin-top:6px; }
        #rlsv-telegram .rlsv-num.rlsv-sm { font-size:22px; }
        #rlsv-telegram .rlsv-num.rlsv-xs { font-size:18px; }
        #rlsv-telegram .rlsv-lbl { font-size:10px; letter-spacing:.06em; text-transform:uppercase; color:#8a97a3; }
        #rlsv-telegram .rlsv-ico { width:30px; height:30px; border-radius:50%; background:#e8f3fd; display:inline-flex; align-items:center; justify-content:center; flex:none; }
        #rlsv-telegram .rlsv-ico svg { display:block; }
        #rlsv-telegram .rlsv-head { display:flex; justify-content:space-between; align-items:flex-start; }
        #rlsv-telegram .rlsv-trend { display:inline-flex; align-items:center; gap:4px; font-size:12px; font-weight:700; padding:3px 8px; border-radius:9px; }
        #rlsv-telegram .rlsv-trend.rlsv-up { color:#27ae60; background:rgba(39,174,96,.12); }
        #rlsv-telegram .rlsv-trend.rlsv-down { color:#f2464e; background:rgba(242,70,78,.12); }
        #rlsv-telegram .rlsv-split { display:flex; }
        #rlsv-telegram .rlsv-split > div { flex:1; display:flex; align-items:center; justify-content:space-between; }
        #rlsv-telegram .rlsv-split > div:last-child { padding-left:14px; border-left:1px solid #edf2f5; }
        #rlsv-telegram .rlsv-split > div:first-child { padding-right:14px; }
        #rlsv-telegram .rlsv-two { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
        #rlsv-telegram .rlsv-two > div + div { border-left:1px solid #edf2f5; padding-left:14px; }
        #rlsv-telegram .rlsv-break { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; padding-top:12px; margin-top:12px; border-top:1px solid #edf2f5; }
        #rlsv-telegram .rlsv-bkit { display:flex; flex-direction:column; align-items:center; gap:5px; }
        #rlsv-telegram .rlsv-bkdot { width:9px; height:9px; border-radius:50%; }
        #rlsv-telegram .rlsv-bkv { font-size:13px; font-weight:700; color:#2b3648; }
        #rlsv-telegram .rlsv-rows > div { display:flex; justify-content:space-between; align-items:center; }
        #rlsv-telegram .rlsv-rows > div + div { margin-top:10px; }
        #rlsv-telegram .rlsv-fid { display:inline-block; margin-left:6px; padding:0 6px; border-radius:8px; background:#edf2f5; color:#46aaf2; font-size:10px; font-weight:700; vertical-align:middle; }
        #rlsv-telegram .rlsv-telegram-legend { border:1px dashed #d3dde3; border-radius:10px; padding:9px 12px; margin-bottom:16px; font-size:12px; color:#465363; background:#fafdff; }
        #rlsv-telegram .rlsv-recent { border:1px solid #e1e9ee; border-radius:12px; padding:12px 14px; margin-bottom:12px; background:#fff; }
        #rlsv-telegram .rlsv-rtype { display:inline-block; padding:2px 10px; border-radius:10px; font-size:12px; background:#e8f3fd; color:#46aaf2; }
        #rlsv-telegram .rlsv-rtype.rlsv-reels { background:#fdeef0; color:#f2464e; }
        #rlsv-telegram .rlsv-rtype.rlsv-story { background:#fef0dd; color:#b5670a; }
        #rlsv-telegram .rlsv-rmini { display:flex; flex-wrap:wrap; gap:6px 16px; margin-top:8px; font-size:12px; color:#5b6b7d; }
        #rlsv-telegram .rlsv-muted { color:#8a97a3; font-size:12px; }
        @media (max-width:480px){ #rlsv-telegram .rlsv-two { grid-template-columns:1fr; } #rlsv-telegram .rlsv-two > div + div { border-left:none; padding-left:0; border-top:1px solid #edf2f5; padding-top:10px; } }
    </style>
    ...
    {else}
        ...
        {if $rlsv_tg.media}
            ...
            {foreach from=$rlsv_tg.media item="m"}
...
{capture name="mainbox_title"}{__("rlsv_autopost.telegram_analytics")}{/capture}
